...
parser.add_argument('--ttf_path', type=str, default='ttf_folder_train',help='ttf directory')
parser.add_argument('--chara', type=str, default='total_chn.txt',help='characters')
parser.add_argument('--save_path', type=str, default='data_train',help='images directory')
...
for idx, (label, item) in enumerate(zip(range(len(all_image_paths)),all_image_paths)):
    ...
    font_name = item.split('/')[-1].split('.')[0].replace('-2', '')
    ...
    for (chara, cnt) in zip(characters, range(len(characters))):
        ...
        path_full = os.path.join(args.save_path, f'id_{label}_{font_name}')
